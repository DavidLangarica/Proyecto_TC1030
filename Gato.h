...
class Gato{
    ...
    public: // Métodos
        ...
        ~Gato(){cout << "\nTerminando el juego... Hasta la proxima!\n" << endl;} // Destructor para volver a iniciar el juego
...
Gato :: Gato(){
    ...
    Trivia pregunta2(2,"Quimica","Que es una mezcla homogenea?","Combinacion de dos o mas sustancias, en las que no se distinguen individualmente y forman una sola fase","Combinacion de dos o mas sustancias, en la que se distinguen individualmente y forman mas de una fase","Combinacion de dos o mas sustancias, que da paso a la vaporización de la sustancia","Combinacion de una sustancia con otra sustancia molecularmente similar");
...
void Gato :: bienvenidatxt(){
    ...
    cout << "\nJugaras contra la computadora. Pero, para hacer mas retador el programa, se haran uso de preguntas de trivia. Si respondes correctamente (tienes 2 oportunidades), entonces ganas un turno y podras colocar tu marca, en caso de responder incorrectamente las 2 oportunidades, perderas tu turno en esa ronda. Comencemos..." << endl;
};
...
bool Gato :: preguntacasilla(int pos_){
    // Método para mostrar todas las preguntas dependiendo de la casilla y evaluar si son correctas.
    int nintentos = 2; // El usuario tiene 2 intentos para contestar correctamente.
        ...
        for (int i = 0; i < nintentos-1; i++){
            do{
                ...
                if (ans == 'B'){ 
                    ...
                    if (nintentos>0){
                        // Si todavia no ha gastado sus 2 intentos, puede intentarlo nuevamente
                        cout << "\nIntenta nuevamente, te queda otro intento" << endl;
                    }
                    else{
                        // Se gastó todos sus intentos, pasa de turno
